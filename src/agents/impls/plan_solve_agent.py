...
class PlanAndSolveAgent(Agent):
    ...
    def run_with_planning(self, input_text: str, **kwargs) -> Dict[str, Any]:
        """带详细规划的运行方法"""

        # 生成计划
        plan = self._generate_plan(input_text)

        # 执行计划
        execution_result = self._execute_plan(plan, input_text)
...
{{
    ...
    "steps": [
        ...
        plan_response = await self.llm.invoke([{"role": "user", "content": planning_prompt}])
...
{{
    ...
    "steps": [
        ...
        plan_response = self.llm.invoke([{"role": "user", "content": planning_prompt}])
    ...
    def _parse_text_plan(self, plan_text: str, query: str) -> Dict[str, Any]:
        """解析文本格式的计划"""
    ...
    async def _execute_step_async(self, step: Dict[str, Any], original_query: str, previous_results: List) -> Dict[str, Any]:
        """异步执行单个步骤"""
...
请完成这个步骤。请提供清晰、准确的回答。"""
        ...
        try:
            response = await self.llm.invoke([{"role": "user", "content": step_prompt}])
...
请完成这个步骤。请提供清晰、准确的回答。"""
        ...
        try:
            response = self.llm.invoke([{"role": "user", "content": step_prompt}])
...
"""
        ...
        final_answer = await self.llm.invoke([{"role": "user", "content": integration_prompt}])
        return final_answer
...
"""
        ...
        final_answer = self.llm.invoke([{"role": "user", "content": integration_prompt}])
        return final_answer
    ...
    async def _execute_step_with_tools_async(self, step: Dict[str, Any], original_query: str, previous_results: List) -> Dict[str, Any]:
        """异步执行单个步骤（支持工具调用）"""
